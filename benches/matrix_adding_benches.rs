...
fn bench_matrix_vectored_add_in_place(c: &mut Criterion) {
    let gpu_math = GpuMath::new();
    ...
    c.bench_function("vectored_add_in_place", |b| {
        b.iter(|| {
...
fn bench_matrix_vectored_add_in_place_big(c: &mut Criterion) {
    let gpu_math = GpuMath::new();
    ...
    c.bench_function("vectored_add_in_place_big", |b| {
        b.iter(|| {
            Matrix::vectored_add_in_place(&mat, &vec).expect("Failed");
        });
    });
...
criterion_group!(
    ...
    bench_matrix_vectored_add_in_place,
    bench_matrix_vectored_add_in_place_big
);
